...
router.post('/:id', authenticateToken, upload.any(), async (req, res) => {
    try {
        const { 
            name, brand, batchNumber, mfgDate, expDate, mrp, dynamicData, isActive,
            categoryId, subcategory, supportPhone, supportWhatsapp
        } = req.body;
        ...
        // Handle photos from upload.any()
        const photoFiles = req.files ? req.files.filter(f => f.fieldname === 'photos') : [];
        if (photoFiles.length > 0) {
            const newPhotos = photoFiles.map(file => `/uploads/photos/${file.filename}`);
...
router.post('/', authenticateToken, upload.fields([{ name: 'photos', maxCount: 5 }]), async (req, res) => {
    try {
        ...
        res.json({ ...product, qrUrl: `/scan/${productCode}` });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
router.delete('/:id', authenticateToken, async (req, res) => {
    try {
        // Delete all associated scan logs first to avoid foreign key constraint errors
        await prisma.productScanLog.deleteMany({
