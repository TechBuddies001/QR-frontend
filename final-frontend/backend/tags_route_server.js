...
router.put('/:id', authenticateToken, upload.fields([{ name: 'photo', maxCount: 1 }, { name: 'photos', maxCount: 5 }]), async (req, res) => {
  try {
    ...
    // Handle Photos (Gallery) - Merge existing with new
    const currentTag = await prisma.tag.findUnique({ where: { id: req.params.id } });
    let finalPhotos = [];
    if (currentTag.photos) {
      try { finalPhotos = JSON.parse(currentTag.photos); } catch(e) {}
    }

    // If frontend sends keepPhotos, filter existing ones (allows deletion)
    if (req.body.keepPhotos) {
      try {
        const keepList = JSON.parse(req.body.keepPhotos);
        finalPhotos = finalPhotos.filter(p => keepList.includes(p));
      } catch(e) {}
    }

    // Append new photos
    if (req.files?.photos) {
      const newPaths = req.files.photos.map(f => `/uploads/photos/${f.filename}`);
      finalPhotos = [...finalPhotos, ...newPaths];
    }
    
    if (finalPhotos.length > 0 || req.files?.photos) {
      updateData.photos = JSON.stringify(finalPhotos);
    }
